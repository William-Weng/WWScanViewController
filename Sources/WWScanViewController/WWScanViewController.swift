...
public protocol WWScanDelegate: AnyObject {
...
/// [掃瞄條碼的ViewController](https://ecshweb.pchome.com.tw/search/v3.3/?q=條碼掃描器)
open class WWScanViewController: UIViewController {
    ...
    private weak var myDelegate: WWScanDelegate?
...
extension WWScanViewController: AVCaptureMetadataOutputObjectsDelegate {
...
// MARK: - 設定
extension WWScanViewController {
    ...
    ///   - queue: DispatchQueue?
    public func configure(types: [AVMetadataObject.ObjectType], delegate: WWScanDelegate, queue: DispatchQueue? = .main) {
        scanTypes = types
        myDelegate = delegate
        callbackQueue = queue
    }
